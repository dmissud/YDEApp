...
Feature: Note management test
  ...
  Scenario: Referencing a new note to unknown application
    ...
    When User wants to reference a new note to application "AP00002" with following datas
      |noteTitle   |noteContent                |noteCreationDate|
      |The note    |Once upon a time...        |30/02/2020      |
    ...
    Scenario: Updating an existing note
      ...
      When User wants to update note "The note" with datas
        |noteTitle      |noteContent                |noteCreationDate|
    ...
    Scenario: getting all application notes
      Given An existing application "AP00002" with existing notes "Note 1" and "Note 2"
      When User wants to get a list content with "Note 1" and "Note 2"
